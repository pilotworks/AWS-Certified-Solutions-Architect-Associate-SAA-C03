...
import { Award, RotateCcw, Filter, CheckCircle2 } from 'lucide-react';
    ...
    <div key={resetKey} className="space-y-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          >
            ...
            {answeredCount > 0 && (
              ...
              >
                {scorePercentage}% {scorePercentage >= 72 ? '(Pass)' : '(Fail < 72%)'}
              </span>
            )}
          </div>
      ...
      <div className="flex items-center gap-2 flex-wrap text-xs" style={{ color: 'var(--text-secondary)' }}>
        <span className="flex items-center gap-1 font-semibold">
          <Filter className="w-3.5 h-3.5" style={{ color: 'var(--text-accent)' }} /> Filter:
        </span>
      ...
      {answeredCount > 0 && answeredCount === filteredQuestions.length && (
        <div className="glass-card rounded-2xl p-6 text-center border-amber-500/30 bg-gradient-to-b from-amber-500/10 to-slate-950">
          ...
          <h3 className="text-xl font-bold text-white mb-1">Module Practice Completed!</h3>
          <p className="text-slate-300 text-sm mb-4">
            You scored <strong className="text-amber-400">{correctCount}</strong> out of{' '}
